...
def sendemail(messages):  # 用于签到后发送邮箱提醒
    ...
    msg = MIMEText('今日体温签到情况：' + messages, 'plain', 'GB2312')  # 邮箱正文内容，第一个参数为内容，第二个参数为格式(plain 为纯文本)，第三个参数为编码
...
def tem_daka():
    ...
    driver.get(r"https://app.nwafu.edu.cn/site/ncov/dailyup")
    ...
    driver.find_element_by_xpath("/html/body/div[1]/div[1]/div/section/div[4]/ul/li[4]/div/input").click()  # 获取地理位置
    print('正在获取地理位置...')
    time.sleep( 3 )

    link = driver.find_element_by_xpath("/html/body/div[1]/div[1]/div/section/div[5]/div/a") #获取到gba值
    ...
    if color == "#4285F4":
        ...
        sendemail(message)
        print("体温打卡完成！邮件已发送！程序将在5秒后关闭")
        driver.quit()
    else:
        print("今日已打卡")
        message = '体温打卡失败！您今日已打卡！请勿重复打卡！'
        sendemail(message)
        print("体温打卡失败！邮件已发送！程序将在5秒后关闭")
        driver.quit()
def countdown(n):
    while n>=0:
        time.sleep(1)
